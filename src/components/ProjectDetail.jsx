...
import { getPrimaryProjectImage, normalizeProjectImages } from "../utils/projectImages";
...
const ProjectDetails = () => {
      ...
      try {
        ...
        if (found && mounted) {
          ...
          const enhanced = {
            ...
            Img: getPrimaryProjectImage(found),
        ...
        if (selectedProject && mounted) {
          ...
          const enhancedProject = {
            ...
            Title: normalizedTitle,
            Description: normalizedDescription,
            Img: getPrimaryProjectImage(selectedProject),
            Images: normalizedImages,
  ...
  const primaryImage = projectImages[0] || project.img || project.Img || "";
                  ...
                  <div className="space-y-3 md:space-y-4">
                    <div className="relative rounded-2xl overflow-hidden border border-white/10 shadow-2xl group">
                      ...
                      {projectImages.length > 1 && (
                        <div className="absolute top-4 right-4 px-2.5 py-1 rounded-full bg-black/60 backdrop-blur-sm border border-white/10 text-[11px] text-white">
                    ...
                    {projectImages.length > 1 && (
                      <div className="grid grid-cols-2 gap-3 md:gap-4">
                        {projectImages.slice(1).map((image, index) => (
                          <div key={`${image}-${index}`} className="relative overflow-hidden rounded-xl border border-white/10 bg-white/5">
                            <img
                              src={image}
                              alt={`${project.Title || project.title} image ${index + 2}`}
                              className="w-full h-36 md:h-44 object-cover transition-transform duration-500 hover:scale-105"
                            />
                          </div>
                        ))}
                      </div>
